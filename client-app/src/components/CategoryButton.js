...
export default function CategoryButton(props) {
    ...
    function setCurrentChannel() {
        ...
        props.socket.emit('join-channel', props.uid, props.PeerId, props.type);
    }
    ...
    return (
        <>
            ...
            {channelType === 'voice' && (
                <>
                    <div className="channel-select-btn noselect" onClick={() => joinVoice(id)}>
                        <FontAwesomeIcon icon={faVolumeUp} className="icon" />{name}
                    </div>
                    {peer.map((p) => (
